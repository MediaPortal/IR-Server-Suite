  ...
  class DriverXP : Driver
  {
    ...
    public override void Send(IrCode code, int port)
    {
      ...
      */
      ...
      /*
      while (data.Length > 255) // use 255 to catch the "0x8x" segment boundaries
      {
        byte[] segment = new byte[255];
        Array.Copy(data, segment, segment.Length);

        WriteSync(segment);

        byte[] newData = new byte[data.Length - 255];
        Array.Copy(data, 255, newData, 0, newData.Length);
        data = newData;
      }
      */
