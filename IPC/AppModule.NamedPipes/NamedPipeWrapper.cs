...
namespace AppModule.NamedPipes {
	...
	public sealed class NamedPipeWrapper {
		...
		public static byte[] ReadBytes(PipeHandle handle, int maxBytes) {
			...
			if (NamedPipeNative.ReadFile(handle.Handle, intBytes, 4, numReadWritten, IntPtr.Zero))
      {
				len = BitConverter.ToInt32(intBytes, 0);
				msgBytes = new byte[len];
				// Read the rest of the data
        if (!NamedPipeNative.ReadFile(handle.Handle, msgBytes, (uint)len, numReadWritten, IntPtr.Zero))
        {
		...
		public static void WriteBytes(PipeHandle handle, byte[] bytes) {
			...
			if (NamedPipeNative.WriteFile(handle.Handle, BitConverter.GetBytes(len), 4, numReadWritten, IntPtr.Zero)) {
				// Write the whole message
				if (!NamedPipeNative.WriteFile(handle.Handle, bytes, len, numReadWritten, IntPtr.Zero)) {
					handle.State = InterProcessConnectionState.Error;
      ...
      handle.Handle = NamedPipeNative.CreateFile(
        name, 
        NamedPipeNative.GENERIC_WRITE, 
        0, 
        null, 
        ...
        if (handle.Handle.ToInt32() != NamedPipeNative.INVALID_HANDLE_VALUE) {
					...
					if (i >= ATTEMPTS) {
						handle.State = InterProcessConnectionState.Error;
						throw new NamedPipeIOException("Error setting read mode on pipe " + name + " . Internal error: " + Marshal.GetLastWin32Error().ToString(), Marshal.GetLastWin32Error());
					}
				}
				if (i >= ATTEMPTS) {
					if (Marshal.GetLastWin32Error() != NamedPipeNative.ERROR_PIPE_BUSY) {
						handle.State = InterProcessConnectionState.Error;
						// After a certain number of unsuccessful attempt raise an exception
						throw new NamedPipeIOException("Error connecting to pipe " + name + " . Internal error: " + Marshal.GetLastWin32Error().ToString(), Marshal.GetLastWin32Error());
					} else {
						handle.State = InterProcessConnectionState.Error;
      ...
      if (NamedPipeNative.CreateSaWithNullDaclSd(out sa))
      {
        pSd = Marshal.AllocHGlobal(Marshal.SizeOf(sa));
        Marshal.StructureToPtr(sa, pSd, true);
      }
      ...
      for (int i = 1; i<=ATTEMPTS; i++)
      {
				...
				handle.Handle = NamedPipeNative.CreateNamedPipe(
					name,
          NamedPipeNative.PIPE_ACCESS_INBOUND,
		...
		public static void Connect(PipeHandle handle) {
			...
			if (!connected && Marshal.GetLastWin32Error() != NamedPipeNative.ERROR_PIPE_CONNECTED) {
				handle.State = InterProcessConnectionState.Error;
		...
		public static uint NumberPipeInstances(PipeHandle handle) {
			...
			if (NamedPipeNative.GetNamedPipeHandleState(handle.Handle, IntPtr.Zero, ref curInstances, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, 0)) {
				return curInstances;
			} 
			else {
