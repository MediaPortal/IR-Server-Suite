      ...
      try
      {
        switch (received.Name)
        {
          ...
          case "Learn Success":
            {
              if (LogVerbose)
                Log.Info("TV3BlasterPlugin: Learned IR Successfully \"{0}\"", _learnIRFilename);
          ...
          case "Server Shutdown":
            {
              Log.Error("TV3BlasterPlugin: IR Server Shutdown - Plugin disabled until IR Server returns");
              _registered = false;
              break;
            }
          ...
          case "Error":
            {
              Log.Error("TV3BlasterPlugin: Received error: {0}", Encoding.ASCII.GetString(received.Data));
              break;
            }

          default:
            {
              Log.Debug("TV3BlasterPlugin: Unknown message received: {0}", received.Name);
              break;
            }
        }
